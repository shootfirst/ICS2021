...
//   sword_t simm = (s->isa.instr.b.imm11 << 11) 
...
//   def_INSTR_TAB("??????? ????? ????? 101 ????? ????? ??", sra_srl_divu);
//   return EXEC_ID_inv;
// }

// def_THelper(calculate_imm) {
...
//   //************************************pa2*******************************************
//   def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , calculate_imm);
